...
@onready var interact_ray: RayCast3D = $Head/RayCast3D
...
func _physics_process(delta):
	...
	if collider and collider.has_method("hover"):
		collider.hover()
	
	if physics_locked:
		return
	
	if is_valid_input and not movement_locked:
...
func _on_hurt(_amount: int):
	...
	if hurt_timer:
		...
		return
	SignalBus.player_set_invulnerable(invuln_length)
